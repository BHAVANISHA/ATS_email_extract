...
class field_glass_zebra:
    ...
    def field_glass_zebra_extract_information(self):
        global extracted_list
        labels = "fieldglass_zebra"
        ...
        for email_data in unread_emails:
            ...
            if ' Time Sheet completely approved' in email_subject:
                if '### Worker Name' in email_body:
                    candidate_name = email_body.split( '### Worker Name', 1 )[1].split( '### Time Sheet ID', 1 )[0].strip()
                ...
                if '### Time Sheet ID' in email_body:
                    time_sheet_id = email_body.split( '### Time Sheet ID', 1 )[1].split( '### Time Sheet Start Date', 1 )[0].strip()
                    result_data_item['time_sheet_id'] = time_sheet_id
                if '### Time Sheet Start Date' in email_body:
                    start_date = email_body.split( '### Time Sheet Start Date', 1 )[1].split( '### Time Sheet End Date', 1 )[0].strip()
                ...
                if '### Time Sheet End Date' in email_body:
                    end_date = email_body.split( '### Time Sheet End Date', 1 )[1].split('### Main Document ID',1)[0].strip()
                ...
                if '### Main Document ID' in email_body:
                    document_id = email_body.split( '### Main Document ID', 1 )[1].split( '### Facility', 1 )[0].strip()
                    result_data_item['document_id'] = document_id
                    comments.append( 'document_id:' + document_id )
                if '### Facility' in email_body:
                    facility = email_body.split( '### Facility', 1 )[1].split( '### Worker ID', 1 )[0].strip()
                    result_data_item['facility'] = facility
                    comments.append( 'facility:' + facility )
                if '### Worker ID' in email_body:
                    worker_id = email_body.split( '### Worker ID', 1 )[1].split( '### Work Order ID', 1 )[0].strip()
                    result_data_item['worker_id'] = worker_id
                    comments.append( 'worker_id:' + worker_id )
                if '### Work Order ID' in email_body:
                    worker_order_id = email_body.split( '### Work Order ID', 1 )[1].split( '### Time Sheet Billable Days', 1 )[0].strip()
                    result_data_item['worker_order_id'] = worker_order_id
                    comments.append( 'worker_order_id:' + worker_order_id )
                if '### Time Sheet Billable Days' in email_body:
                    billable_days = email_body.split( '### Time Sheet Billable Days', 1 )[1].split( '### Time Sheet Billable Hours', 1 )[0].strip()
                    result_data_item['billable_days'] = billable_days
                    comments.append( 'billable_days:' + billable_days )
                if '### Time Sheet Billable Hours' in email_body:
                    billable_hours = email_body.split( '### Time Sheet Billable Hours', 1 )[1].split( '### Time Sheet Total Quantity', 1 )[0].strip()
...
results = field_glass_zebra()
results.field_glass_zebra_extract_information()
