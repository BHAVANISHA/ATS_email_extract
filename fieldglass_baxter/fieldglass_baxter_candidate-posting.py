...
class field_glass_baxter_interviewing:
    ...
    def fieldglass_baxter_extract_information(self):
        ...
        for email_data in unread_emails:
            ...
            if '### Interview / Meeting Subject' in email_body:
                meet_sub = \
                    email_body.split( '### Interview / Meeting Subject', 1 )[1].strip( ' \n' ).split( '### Start Time',
                                                                                                      1 )[
                        0].strip()
            ...
            if '### Start Time' in email_body:
                week_start = \
                    email_body.split( '### Start Time', 1 )[1].split( '### End Time', 1 )[
                        0].strip()
                result_data_item['week_start'] = week_start

            if '### End Time' in email_body:
                end_date = email_body.split( '### End Time', 1 )[1].strip().split(
                    '### Interview / Meeting Time Zone', 1 )[
                    0].strip()

                result_data_item['week_end'] = end_date

            if '### Interview / Meeting Time Zone' in email_body:
                meet_time = email_body.split( '### Interview / Meeting Time Zone', 1 )[1].split(
                    '### Preferred Interview / Meeting Type', 1 )[
                    0].strip()
                # result_data_item['meet_time'] = meet_time
                comments.append('meet_time:'+meet_time)

            if '### Preferred Interview / Meeting Type' in email_body:
                meet_type = \
                    email_body.split( '### Preferred Interview / Meeting Type', 1 )[1].strip( ' \n' ).split(
                        '### Details',
                        1 )[0].strip()
            ...
            if 'Kannepalli, Venkata Srilakshmi - USA -' in email_body:
                job_title = email_body.split( 'Kannepalli, Venkata Srilakshmi - USA -', 1 )[1].strip().split( '(', 1 )[0].strip()
                result_data_item['job_title'] = job_title

            if ' Technical Writer III (' in email_body:
                client_jobid = email_body.split( ' Technical Writer III (', 1 )[1].strip( ' \n' ).split( ')', 1 )[
                    0].strip()
                result_data_item['client_jobid'] = client_jobid.rstrip( ' \n' ).strip()
            if '### Phone Number' in email_body:
                phone_num = email_body.split( '### Phone Number', 1 )[1].strip( ' \n' ).split( '### Interviewer', 1 )[
                    0].strip()
                comments.append('phone_num:'+phone_num)
            if '### Interviewer' in email_body:
                interviewer = email_body.split( '### Interviewer', 1 )[1].strip( ' \n' ).split( '![Details]', 1 )[
                    0].strip()
...
results = field_glass_baxter_interviewing()
results.fieldglass_baxter_extract_information()
