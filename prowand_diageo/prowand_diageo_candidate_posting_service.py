...
class prowand_diageo_interviewing:
    ...
    def prowand_diageo_extract_information(self):
        # global email_subject, result
        labels = "prowand_diageo"
        ...
        for email_data in unread_emails:
            ...
            if 'Cask Handler -' in email_data['subject']:
                first_name = email_data['subject'].split( 'Cask Handler -', 1 )[1].strip( ' \n' ).split( ')', 1 )[
                    0].strip()
                result_data_item['first_name'] = first_name.rstrip( ' \n' ).strip()

            if 'for the' in email_body:
                job_title = email_body.split( 'for the', 1 )[1].strip().split( '.', 1 )[0].strip()
                result_data_item['job_title'] = job_title

            if 'Requisition #' in email_body:
                client_jobid = email_body.split( 'Requisition #', 1 )[1].strip().split( '\n', 1 )[
                    0].strip()
                result_data_item['client_jobid'] = client_jobid

            if 'Notes: ' in email_body:
                notes = email_body.split( 'Notes: ', 1 )[1].strip( ' \n' ).split( 'If you have any questions', 1 )[
                    0].strip()
...
results = prowand_diageo_interviewing()
results.prowand_diageo_extract_information()
