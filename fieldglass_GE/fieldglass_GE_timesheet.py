...
class field_glass_GE:
        ...
        try:
            ...
            for result in extracted_list:
                ...
                if existing_record:
                    ...
                    data_to_update = (comments, week_end, time_sheet_id, placement_id)
    ...
    def field_glass_GE_extract_information(self):
        ...
        for email_data in unread_emails:
            ...
            if 'Time Sheet completely approved'in email_subject:
                if '### Worker Name' in email_body:
                    candidate_name = \
                    email_body.split( '### Worker Name', 1 )[1].strip( ' \n' ).split( '### Time Sheet ID', 1 )[0].strip()
                    result_data_item['candidate_name'] = candidate_name.rstrip( '- ' ).strip()
                    comments.append( 'candidate_name:' + candidate_name.rstrip('- ').strip() )

                if '### Time Sheet ID' in email_body:
                    time_sheet_id = email_body.split( '### Time Sheet ID', 1 )[1].split( '### Time Sheet End Date', 1 )[
                        0].strip()
                    result_data_item['time_sheet_id'] = time_sheet_id

                if '### Time Sheet End Date' in email_body:
                    end_date = email_body.split( '### Time Sheet End Date', 1 )[1].split( '### Worker Location', 1 )[
                        0].strip()
                ...
                if '### Worker Location' in email_body:
                    site = email_body.split( '### Worker Location', 1 )[1].split( '### Time Sheet Total Quantity', 1 )[0].strip()
                    result_data_item['site'] = site
                    comments.append( 'site:' + site )
                if '### Time Sheet Total Quantity' in email_body:
                    total_quantity = \
                    email_body.split( '### Time Sheet Total Quantity', 1 )[1].replace('\n---','').replace(' ','').split( '![Details]',1 )[0].replace('\n','')
                    result_data_item['total_quantity'] = total_quantity.rstrip( ' -\n' ).strip()
...
results = field_glass_GE()
results.field_glass_GE_extract_information()
